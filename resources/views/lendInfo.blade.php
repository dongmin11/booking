    ...
    @csrf
        ...
        <select name="LenderName" require>
            @foreach($users as $user)
                <option <?php if($UserName == $user['name']){ echo 'selected';} ?> value="<?php if(isset($user['name'])){echo $user['name'];} ?>"><?php if(isset($user['name'])){echo $user['name'];}  
                ?></option>
            @endforeach
            </select>
        </div>
        ...
        <div>
            <p>貸出日</p>
            <input type="date" name="LendingDate" required value="<?php echo date('Y-m-d'); ?>" min="<?php echo date('Y-m-d');?>"  max="<?php echo date('Y-m-d',strtotime('+1 month'));?>">
        </div>

        <div>
            <p>返却予定日</p>
            <input type="date" name="ReturnExpectedDate" required min="<?php echo date('Y-m-d');?>" max="<?php echo date('Y-m-d',strtotime('+1 month'));?>">
        </div>
